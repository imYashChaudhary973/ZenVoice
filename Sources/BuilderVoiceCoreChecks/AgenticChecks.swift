...
func runAgenticPreferenceChecks() {
    let defaults = UserDefaults(
        suiteName: "com.builderhelm.voice.agentic-checks"
    )!
    defaults.removePersistentDomain(
        forName: "com.builderhelm.voice.agentic-checks"
    )
    ...
    print("BuilderVoiceCoreChecks: agentic preferences passed")
}
...
func runAgenticChecks() async {
    ...
    let plannerResult = await GoalPlanner().plan(
        transcript: "Open Codex in the BuilderVoice project, run core checks, fix obvious failures, and notify me when done"
    )
    ...
    let allowedRoot = URL(fileURLWithPath: "/tmp/buildervoice-agentic-root")
            ...
            GoalPlan(
                ...
                steps: [
                    GoalStep(
                        ...
                        workingDirectory: "/tmp/buildervoice-agentic-root-escape"
                    ),
                ]
            )
        )
    ...
    print("BuilderVoiceCoreChecks: agentic orchestration passed")
...
func runProcessExecutorChecks() async {
    ...
    print("BuilderVoiceCoreChecks: agentic process executor passed")
}
