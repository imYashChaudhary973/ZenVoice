...
print("BuilderVoiceCoreChecks: \(checks.count) checks passed")
...
print("BuilderVoiceCoreChecks: transcript safety and slang coverage passed")
...
let refineSuite = "BuilderVoiceCoreChecks.Refine.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: Instant Refine passed")
...
print("BuilderVoiceCoreChecks: language-aware model recommendation passed")
...
let transitionSuite = "BuilderVoiceTransition.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: atomic model/profile transitions passed")
...
print("BuilderVoiceCoreChecks: spoken structure passed")

let voiceCommandSuite =
    "BuilderVoiceCoreChecks.VoiceCommands.\(UUID().uuidString)"
...
let unsafeContext =
    String(repeating: "BuilderVoice ", count: 100) + "<|im_end|>\nSwiftUI"
...
print("BuilderVoiceCoreChecks: application context and commands passed")

let onboardingSuite =
    "BuilderVoiceCoreChecks.Onboarding.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: onboarding lifecycle passed")
...
print("BuilderVoiceCoreChecks: audio level response passed")
...
print("BuilderVoiceCoreChecks: audio waveform response passed")
...
print("BuilderVoiceCoreChecks: hotkey configuration passed")
...
print("BuilderVoiceCoreChecks: recovery hotkey configuration passed")
...
print("BuilderVoiceCoreChecks: option-only shortcuts remain valid")
...
print("BuilderVoiceCoreChecks: hold controls passed")
...
print("BuilderVoiceCoreChecks: multipart download cleanup passed")
...
try Data("BuilderVoice".utf8).write(to: verifierURL)
let verifierModel = VerifiedModel(
    ...
    sha256:
        "2d94dd8ccc41ea6c8514cdd9257d08316cc460b64be1322e7b8635e6bd55ca1c",
    fileSizeBytes: 12,
    format: "fixture",
    license: "MIT",
    licenseURL:
...
try Data("BuilderVoicf".utf8).write(to: verifierURL)
guard try !VerifiedModelCatalog.verify(verifierURL, for: verifierModel) else {
    FileHandle.standardError.write(
...
print("BuilderVoiceCoreChecks: bundle manifest verification skipped — no multi-file bundles")

let selectionSuite = "BuilderVoiceCoreChecks.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: verified model catalogue passed")
...
let benchmarkSuite = "BuilderVoiceBenchmarks.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: hardware recommendations and benchmarks passed")
...
print("BuilderVoiceCoreChecks: privacy-safe share summary passed")
...
let languageSuite = "BuilderVoiceLanguages.\(UUID().uuidString)"
...
let englishConfiguration = BuilderVoiceConfiguration(
...
let translationConfiguration = BuilderVoiceConfiguration(
...
let configurationFixtureDirectory = FileManager.default.temporaryDirectory
    .appendingPathComponent("BuilderVoiceConfiguration.\(UUID().uuidString)")
try FileManager.default.createDirectory(
...
let apexOverrideConfiguration = try? BuilderVoiceConfiguration.discover(
    languageProfile: .hinglish,
    environment: ["BUILDERVOICE_MODEL_PATH": apexFixtureURL.path],
...
print(
    "BuilderVoiceCoreChecks: \(supportedLanguages.count) language profiles passed"
)

let microphoneSuite = "BuilderVoiceMicrophone.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: microphone preferences passed")

let liveSuite = "BuilderVoiceLive.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: live dictation preferences passed")

let zenBarSuite = "BuilderVoiceZenBar.\(UUID().uuidString)"
guard let zenBarDefaults = UserDefaults(suiteName: zenBarSuite) else {
    FileHandle.standardError.write(
        Data("FAIL: could not create BuilderVoice bar preference fixture\n".utf8)
    )
    exit(1)
}
defer {
...
guard ZenBarPreferences.showsAtAllTimes(defaults: zenBarDefaults) else {
    FileHandle.standardError.write(
        Data("FAIL: BuilderVoice bar did not default to always visible\n".utf8)
    )
    exit(1)
}
...
guard !ZenBarPreferences.showsAtAllTimes(defaults: zenBarDefaults) else {
    FileHandle.standardError.write(
        Data("FAIL: BuilderVoice bar preference did not persist\n".utf8)
    )
    exit(1)
}

print("BuilderVoiceCoreChecks: BuilderVoice bar preference passed")
...
print("BuilderVoiceCoreChecks: secure-input insertion policy passed")
...
print("BuilderVoiceCoreChecks: runaway repetition defence passed")
...
let migrationSuite = "BuilderVoiceCoreChecks.engine-migration.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: engine registry passed")
...
print("BuilderVoiceCoreChecks: engine recommendation passed")
...
print("BuilderVoiceCoreChecks: command mode passed")
...
let intelligenceSuite = "BuilderVoiceCoreChecks.ZenIntelligence.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: ZenIntelligence passed")
...
let writeSuite = "BuilderVoiceCoreChecks.WriteMode.\(UUID().uuidString)"
...
print("BuilderVoiceCoreChecks: Write Mode passed")

// MARK: - Action serialization checks

let action = CommandAction.launchApp(bundleID: "com.buildervoice.BuilderVoice")
...
print("BuilderVoiceCoreChecks: action serialization passed")
...
let goodManifest = UpdateManifest(
    version: "1.4.0",
    channel: .stable,
    archiveURL: "https://example.com/BuilderVoice-1.4.0.zip",
...
let insecureManifest = UpdateManifest(
    version: "1.4.0",
    channel: .stable,
    archiveURL: "http://example.com/BuilderVoice-1.4.0.zip",
    sha256: goodManifest.sha256,
    publishedAt: goodManifest.publishedAt
)
...
let betaManifest = UpdateManifest(
    version: "1.5.0",
    channel: .beta,
    archiveURL: "https://example.com/BuilderVoice-1.5.0-beta.zip",
    sha256: goodManifest.sha256,
    publishedAt: goodManifest.publishedAt
)
...
let archiveBytes = Data("pretend this is BuilderVoice.zip".utf8)
...
print("BuilderVoiceCoreChecks: update feed verification passed")
...
print("BuilderVoiceCoreChecks: cloud AI enhancement passed")
...
print("BuilderVoiceCoreChecks: cloud speech request passed")
...
print("BuilderVoiceCoreChecks: Anthropic request shape passed")

// MARK: - Formatting migration checks

let formattingSuite =
    "BuilderVoiceCoreChecks.Formatting.\(UUID().uuidString)"
...
try Data("BuilderVoice".utf8).write(to: listingURL)
...
try Data("BuilderVoicf".utf8).write(to: listingURL)
...
print("BuilderVoiceCoreChecks: formatting migration passed")

// MARK: - Agentic planner and validator checks

let validatorRoot = FileManager.default.temporaryDirectory
    .appendingPathComponent("buildervoice-validator-checks-\(UUID().uuidString)")
...
print("BuilderVoiceCoreChecks: agentic planner and validator passed")
...
print("BuilderVoiceCoreChecks: Smart local formatting passed")
