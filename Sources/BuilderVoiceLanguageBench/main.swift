...
private struct Options {
    ...
    static func parse(_ arguments: [String]) throws -> Options {
        ...
        var cachePath = FileManager.default.temporaryDirectory
            .appendingPathComponent("buildervoice-language-bench")
            .path
        var limit: Int?
...
private func report(
    ...
    print()
    print("BuilderVoice language benchmark")
    print("model: \(modelID)")
...
private func run() throws {
    ...
    if let catalogModel = VerifiedModelCatalog.model(
        ...
        throw BenchmarkError.invalidArguments(
            "model is not in the verified BuilderVoice catalogue"
        )
    }
    ...
    let configuration = BuilderVoiceConfiguration(
...
do {
    ...
    FileHandle.standardError.write(
        Data("BuilderVoiceLanguageBench failed: \(error.localizedDescription)\n".utf8)
    )
    exit(1)
}
