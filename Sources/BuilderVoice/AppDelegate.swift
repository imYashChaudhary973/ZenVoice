...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private func configureEnginesNow() async {
        ...
        do {
            let configuration = try await Task.detached(
                ...
                try BuilderVoiceConfiguration.discover()
            }.value
    ...
    private func configureMenuBar() {
        ...
        zenBarMenuItem.state =
            state.showsBuilderVoiceAtAllTimes ? .on : .off
    ...
    private func configureZenBar() {
        ...
        state.$phase
            .combineLatest(state.$showsBuilderVoiceAtAllTimes)
    ...
    private func configureSettingsWindow() {
        modelManagerViewModel = ModelManagerViewModel(
            applySelection: { [weak self] model, profile in
                guard let self else {
                    return .failure(
                        BuilderVoiceConfiguration.ConfigurationError.modelMissing
                    )
                }
        ...
        settingsViewModel = SettingsViewModel(
            ...
            showsBuilderVoiceAtAllTimes:
                state.showsBuilderVoiceAtAllTimes,
            ...
            applyZenBarPreference: { [weak self] enabled in
                self?.state.setShowsBuilderVoiceAtAllTimes(enabled)
            },
    ...
    private func applyConfiguration(
        ...
        do {
            ...
            let whisperEngine = try ModelProfileTransition.prepareAndCommit(
                ...
                let configuration = try BuilderVoiceConfiguration.verified(
                    model: model,
                    languageProfile: languageProfile
                )
    ...
    private func finishRecordingNow() async {
        ...
        // complete recording in one pass. BuilderVoiceAccuracyChecks measures the
...
#if DEBUG
        ProcessInfo.processInfo.environment["BUILDERVOICE_E2E_AUTORUN"] == "1"
#else
        false
#endif
    }
    ...
    private func runDeterministicE2EIfRequested() {
        ...
        Task { @MainActor [weak self] in
            ...
            guard self.recorder.isRecording else {
                print("BUILDERVOICE_E2E_RESULT failure recorder-not-started")
            ...
            while Date() < deadline {
                ...
                case .success:
                    ...
                    print(
                        String(
                            format: "BUILDERVOICE_E2E_RESULT success %.3f",
                            elapsed
                        )
                    )
                    await self.finishDeterministicE2E()
                ...
                case .error(let message):
                    print("BUILDERVOICE_E2E_RESULT failure \(message)")
                    await self.finishDeterministicE2E()
                    return
                default:
            ...
            print("BUILDERVOICE_E2E_RESULT failure timeout")
            await self.finishDeterministicE2E()
        }
    }
    ...
    @objc private func toggleZenBar() {
        let enabled = !state.showsBuilderVoiceAtAllTimes
        if let settingsViewModel {
            settingsViewModel.setShowsBuilderVoiceAtAllTimes(enabled)
        } else {
            state.setShowsBuilderVoiceAtAllTimes(enabled)
        }
    }
    ...
    @objc private func overlayPreferencesChanged() {
        ...
        updateZenBarPresentation(
            phase: state.phase,
            showsAtAllTimes: state.showsBuilderVoiceAtAllTimes
        )
    }
