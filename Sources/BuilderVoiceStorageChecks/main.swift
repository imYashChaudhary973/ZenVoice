...
private func checkDeleteAllTranscriptsPreservesIndependentData() async throws {
    let fixture = try await VaultFixture()
    defer { fixture.cleanup() }

    let transcriptID = UUID()
    let transcriptAudio = await fixture.vault.recoveryAudioURL(for: transcriptID)
    try Data("saved audio".utf8).write(to: transcriptAudio)
    try await fixture.vault.begin(
        DictationDraft(
            id: transcriptID,
            language: "en",
            ...
            targetBundleID: nil,
            targetAppName: "Notes",
            recoveryAudioURL: transcriptAudio
        )
    )
    try await fixture.vault.storeTranscript(
        id: transcriptID,
    ...
    try await fixture.vault.deleteRecoveryAudio(id: transcriptID)

    let recoveryID = UUID()
    let recoveryAudio = await fixture.vault.recoveryAudioURL(for: recoveryID)
    try Data("recovery audio".utf8).write(to: recoveryAudio)
    try await fixture.vault.begin(
        DictationDraft(
            id: recoveryID,
            language: "en",
    ...
    try await fixture.vault.deleteAllTranscripts()

    try await require(
        try await fixture.vault.record(id: transcriptID) == nil,
        "transcript-scoped delete left a saved transcript"
    )
    try await require(
        try await fixture.vault.record(id: recoveryID)?.recoveryAudioURL != nil,
        "transcript-scoped delete removed recovery audio"
    )
    try await require(
        FileManager.default.fileExists(atPath: recoveryAudio.path),
        "transcript-scoped delete deleted the recovery file"
    )
    try await require(
        try await fixture.vault.audioArchive(id: archiveID) != nil,
        "transcript-scoped delete removed audio history"
    )
    try await require(
        try await fixture.vault.correctionRules().count == 1,
        "transcript-scoped delete removed correction rules"
    )
    try await require(
        try fixture.keyProvider.loadOrCreateKeyData() == originalKey,
        "transcript-scoped delete rotated the encryption key"
    )
}
...
private func checkHistoryPreferencesDefaults() async throws {
    let suiteName = "BuilderVoiceStorageChecks.\(UUID().uuidString)"
...
private func checkCorrectionEngine() async throws {
    ...
    print(
        String(
            format:
                "BuilderVoiceStorageChecks: personal corrections p50 %.3f ms, p95 %.3f ms",
            p50,
            p95
        )
    )
...
private func checkLocalLearningPreferences() async throws {
    let suite =
        "BuilderVoiceStorageChecks.Learning.\(UUID().uuidString)"
...
private func unzippedManifest(from archiveURL: URL) throws -> String {
    ...
    process.arguments = [
        "-p",
        archiveURL.path,
        "BuilderVoiceAudioHistory/manifest.json"
    ]
    let pipe = Pipe()
...
private func checkAudioHistoryPreferenceDefaults() async throws {
    let suiteName = "BuilderVoiceChecks.audioHistory.\(UUID().uuidString)"
    guard let defaults = UserDefaults(suiteName: suiteName) else {
...
private func checkAgenticTaskPersistence() async throws {
    ...
    let plan = GoalPlan(
        ...
        steps: [
            GoalStep(
                number: 1,
                agent: .codex,
                command: "Run BuilderVoiceCoreChecks and report failures.",
    ...
    record.events.append(
        GoalStatusEvent(
            ...
            event: .stepOutput,
            step: 1,
            message: "BuilderVoiceCoreChecks: distinctive-output-marker"
        )
    )
    ...
    try await require(
        active[0].events.first?.message
            == "BuilderVoiceCoreChecks: distinctive-output-marker",
    ...
    for plaintext in [
        ...
        "Run BuilderVoiceCoreChecks and report failures.",
...
do {
    ...
    print("BuilderVoiceStorageChecks: 25 checks passed")
} catch {
