...
public actor LinkServer {
    ...
    private let queue = DispatchQueue(
        label: "com.builderhelm.voice.link.server"
    )
    ...
    public func start(
        key: LinkPairingKey,
        port requestedPort: UInt16 = BuilderVoiceLink.defaultPort,
        ...
        if advertises {
            listener.service = NWListener.Service(
                name: hostName,
                type: BuilderVoiceLink.bonjourServiceType
            )
        }
    ...
    private func receive(sessionID: UUID) async {
        ...
        session.connection.receive(
            ...
            maximumLength: BuilderVoiceLink.maximumFrameBytes
    ...
    private func handle(_ frame: LinkFrame, on session: Session) async {
        ...
        case .hello(let hello):
            guard hello.protocolVersion == BuilderVoiceLink.protocolVersion else {
                await send(
