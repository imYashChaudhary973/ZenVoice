...
private func makeSilentFixture() throws -> URL {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("buildervoice-runtime-\(UUID().uuidString).wav")
...
/// `BUILDERVOICE_PARAKEET_AUDIO` to a real recording to also require a non-empty
...
do {
    ...
    for configuration: ParakeetTDTEngine.Configuration in [.v3] {
        ...
        guard engine.isLoaded else {
            throw NSError(
                domain: "BuilderVoiceRuntimeChecks",
                code: 7,
                userInfo: [
        ...
        let audioURL =
            try environment["BUILDERVOICE_PARAKEET_AUDIO"].map {
        ...
        let requiresSpeech = environment["BUILDERVOICE_PARAKEET_AUDIO"] != nil
    ...
    FileHandle.standardError.write(
        Data("BuilderVoice Parakeet checks failed: \(error)\n".utf8)
    )
    exit(1)
}
...
do {
    ...
    let languageProfile: LanguageProfile =
        environment["BUILDERVOICE_RUNTIME_LANGUAGE"] == "hinglish"
        ? .hinglish
        : .english
    let configuration = try BuilderVoiceConfiguration.discover(
    ...
    do {
        _ = try transcriber.transcribe(
            ...
            languageProfile: languageProfile,
            initialPrompt: "BuilderVoice SwiftUI"
        )
    } catch WhisperTranscriber.TranscriptionError.noSpeech {
    ...
    print(
        "BuilderVoice runtime checks passed (persistent model + live samples: "
            + "\(transcriber.modelID))."
    )
} catch BuilderVoiceConfiguration.ConfigurationError.modelMissing {
    // Skipping is a local-development convenience only. CI sets
    // BUILDERVOICE_RUNTIME_REQUIRED so a runner without a resolvable model
    // fails the build instead of passing vacuously.
    if ProcessInfo.processInfo.environment["BUILDERVOICE_RUNTIME_REQUIRED"] == "1" {
        let message =
            "BuilderVoice runtime checks failed: no verified model is "
            + "resolvable, but BUILDERVOICE_RUNTIME_REQUIRED is set. Install "
            + "a verified model in Models or set BUILDERVOICE_MODEL_PATH.\n"
        FileHandle.standardError.write(Data(message.utf8))
        exit(1)
    }
    print(
        "BuilderVoice runtime checks skipped: install a verified model in Models."
    )
} catch {
    FileHandle.standardError.write(
        Data("BuilderVoice runtime checks failed: \(error)\n".utf8)
    )
    exit(1)
}
