...
// BuilderVoiceCloudLiveChecks — closes the Phase 6 live-endpoint item.
//
// Deterministic request-shape and response-parse coverage already lives in
// BuilderVoiceCoreChecks against fixtures. What was missing is evidence against
...
//
// Environment:
//   BUILDERVOICE_CLOUD_LIVE_PROVIDER  required: openai | groq | anthropic | custom
//   BUILDERVOICE_CLOUD_LIVE_MODEL     optional; defaults to the provider default
//   BUILDERVOICE_CLOUD_LIVE_BASE_URL  required for custom, optional otherwise
//
// Verifies per run:
...
private func fail(_ message: String) -> Never {
    FileHandle.standardError.write(Data(("BuilderVoiceCloudLiveChecks: \(message)\n").utf8))
    exit(1)
}
...
private func checkWrongKey(
    ...
    let wrongKey = "sk-buildervoice-live-check-deliberately-wrong"
    do {
        let result = try await engine.enhance(
            transcript: messyTranscript,
            configuration: config,
...
guard let providerName = environment["BUILDERVOICE_CLOUD_LIVE_PROVIDER"] else {
    fail(
        "set BUILDERVOICE_CLOUD_LIVE_PROVIDER to openai, groq, anthropic, "
...
let model: String
if let requested = environment["BUILDERVOICE_CLOUD_LIVE_MODEL"] {
    model = requested
} else if let defaultModel = provider.defaultModel {
    model = defaultModel
} else {
    fail("custom requires BUILDERVOICE_CLOUD_LIVE_MODEL")
}
let baseURL = environment["BUILDERVOICE_CLOUD_LIVE_BASE_URL"]

if provider == .custom, baseURL == nil {
    fail("custom requires BUILDERVOICE_CLOUD_LIVE_BASE_URL")
}
...
do {
    guard let key = try storedAPIKey() else {
        fail(
            "no API key is stored. Open BuilderVoice → Formatting → Cloud, "
...
if passed {
    report(
        "BuilderVoiceCloudLiveChecks passed (\(provider.displayName), \(model))."
    )
    exit(0)
} else {
    report("BuilderVoiceCloudLiveChecks FAILED (\(provider.displayName)).")
    exit(1)
}
