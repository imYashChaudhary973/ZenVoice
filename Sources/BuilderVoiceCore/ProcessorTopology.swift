...
public enum ProcessorTopology {
    ...
    /// coordination cost outweighs the extra parallelism on the models BuilderVoice
    /// ships.
    ///
    /// `BUILDERVOICE_DECODE_THREADS` overrides the choice, so the question above
    ...
    public static var decodeThreadCount: Int32 {
        if let override = ProcessInfo.processInfo
            .environment["BUILDERVOICE_DECODE_THREADS"]
            .flatMap(Int.init),
           override > 0 {
