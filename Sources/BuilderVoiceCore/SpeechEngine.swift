...
/// A transcription runtime that BuilderVoice can select and use.
///
/// Conforming types live in `BuilderVoiceRuntime` because they depend on their
/// own frameworks (`whisper`, `Speech`, Core ML, etc.). The protocol itself is
/// in `BuilderVoiceCore` so the app and settings UI can reason about engines
