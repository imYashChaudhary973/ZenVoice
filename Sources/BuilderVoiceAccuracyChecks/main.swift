...
import BuilderVoiceRuntime

// BuilderVoiceAccuracyChecks — measures transcription accuracy instead of guessing
// at it.
//
...
//
// Environment:
//   BUILDERVOICE_MODEL_PATH        explicit model, otherwise the usual discovery
...
//   BUILDERVOICE_ACCURACY_CLEAN    set to 1 to measure studio-clean audio instead
//   BUILDERVOICE_ACCURACY_FIXTURES cache directory for rendered audio
//   BUILDERVOICE_ACCURACY_CORPUS   flat audio/text directory or locked JSONL
//   BUILDERVOICE_ACCURACY_VERBOSE  set to 1 to print every hypothesis
//   BUILDERVOICE_ACCURACY_MAX_SYNTHETIC_CLIPS
//                               cap synthetic clips; real/long-form stay intact
//   BUILDERVOICE_ACCURACY_ENGINE    baseline one engine on the real corpus
//                               through its own SpeechEngine path ("list"
//                               prints the registry's engine ids)
//   BUILDERVOICE_SCORING_ONLY      validate deterministic scoring without a model
//   BUILDERVOICE_CORPUS_VALIDATE_ONLY
...
private func fail(_ message: String) -> Never {
    FileHandle.standardError.write(
        Data("BuilderVoice accuracy checks failed: \(message)\n".utf8)
    )
    exit(1)
}

private func skip(_ message: String) -> Never {
    if flag("BUILDERVOICE_ACCURACY_REQUIRED") {
        fail(message)
    }
    print("BuilderVoice accuracy checks skipped: \(message)")
    exit(0)
}
...
private func validateScoring() {
    ...
    let fixtureDirectory = repository
        ...
        .appendingPathComponent(
            "BuilderVoiceAccuracyChecks-\(UUID().uuidString)",
            isDirectory: true
        )
    do {
...
private func validateCorpusInput() -> Bool {
    guard let path = environment["BUILDERVOICE_ACCURACY_CORPUS"] else {
        fail("BUILDERVOICE_ACCURACY_CORPUS is required for corpus validation")
    }
    ...
    do {
        ...
        report("BuilderVoiceAccuracyChecks corpus validation passed: \(clips.count) clips")
        return true
    } catch {
...
private func discoverConfiguration() -> BuilderVoiceConfiguration {
    do {
        let overrideCapability = environment["BUILDERVOICE_MODEL_PATH"]
            .flatMap {
                VerifiedModelCatalog.model(
                    filename: URL(fileURLWithPath: $0).lastPathComponent
                )?.languageCapability
            }
        return try BuilderVoiceConfiguration.discover(
            languageProfile:
                overrideCapability == .hinglish ? .hinglish : .english
        )
    } catch BuilderVoiceConfiguration.ConfigurationError.modelMissing {
        skip(
            "install a verified model in Models or set BUILDERVOICE_MODEL_PATH."
        )
    } catch {
...
private func makeEngineRegistry(
    configuration: BuilderVoiceConfiguration
) -> EngineRegistry {
...
private func runRealSpeechSmoke() -> Bool {
    guard let corpusPath = environment["BUILDERVOICE_ACCURACY_CORPUS"] else {
        fail("BUILDERVOICE_ACCURACY_SMOKE requires BUILDERVOICE_ACCURACY_CORPUS")
    }
    let corpus = (
    ...
    report("BuilderVoiceAccuracyChecks smoke passed.")
    return true
}
...
/// descriptor id `BUILDERVOICE_ACCURACY_ENGINE` accepts.
...
private func measureEngineBaseline(engineID: String) -> Bool {
    guard let corpusPath = environment["BUILDERVOICE_ACCURACY_CORPUS"] else {
        fail("BUILDERVOICE_ACCURACY_ENGINE requires BUILDERVOICE_ACCURACY_CORPUS")
    }
    let corpus = (
        try? Fixtures.corpus(at: URL(fileURLWithPath: corpusPath))
    ) ?? []
    ...
    let verbose = flag("BUILDERVOICE_ACCURACY_VERBOSE")
    let skipSegments = flag("BUILDERVOICE_ACCURACY_NOSEGMENT")
    ...
    Task.detached {
        ...
        for clip in corpus {
            ...
            let wholeURL = FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "buildervoice-baseline-\(engineID)-whole-\(clip.name).wav"
                )
            ...
            if !skipSegments {
                ...
                for (index, segment) in segments.enumerated() {
                    let segmentURL = FileManager.default.temporaryDirectory
                        .appendingPathComponent(
                            "buildervoice-baseline-\(engineID)"
    ...
    report(
        "BuilderVoiceAccuracyChecks engine baseline passed "
        ...
        for availability in available {
            ...
            for clip in clips {
                ...
                let tempURL = FileManager.default.temporaryDirectory
                    .appendingPathComponent(
                        "buildervoice-engine-\(availability.engine.id)"
                            + "-\(clip.name).wav"
                    )
...
private func probeSpokenStructure() -> Bool {
    guard let configuration = try? BuilderVoiceConfiguration.discover(
        languageProfile: .english
    ) else {
        skip("no speech model available.")
    }
    let directory = environment["BUILDERVOICE_ACCURACY_FIXTURES"]
        .map { URL(fileURLWithPath: $0) }
        ?? FileManager.default.temporaryDirectory
            .appendingPathComponent("buildervoice-accuracy-fixtures")
...
private func evaluateTextCorpus(path: String) -> Bool {
    ...
    let mode = environment["BUILDERVOICE_TEXTEVAL_MODE"] == "agentPrompt"
...
private func measure() -> Bool {
    ...
    let fixtureDirectory = environment["BUILDERVOICE_ACCURACY_FIXTURES"]
        .map { URL(fileURLWithPath: $0) }
        ?? FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "buildervoice-accuracy-fixtures",
                isDirectory: true
            )
    ...
    let isClean = flag("BUILDERVOICE_ACCURACY_CLEAN")
    let isVerbose = flag("BUILDERVOICE_ACCURACY_VERBOSE")
    let gain = isClean ? 1 : value("BUILDERVOICE_ACCURACY_GAIN", default: 0.35)
    let noise = isClean ? 0 : value("BUILDERVOICE_ACCURACY_NOISE", default: 0.004)
    ...
    // error rate between identical runs. Set BUILDERVOICE_ACCURACY_SAMPLED=1 to
    // measure the shipping configuration, temperature fallback included.
    let isReproducible = !flag("BUILDERVOICE_ACCURACY_SAMPLED")
    ...
    let latencyRepeats = Int(
        environment["BUILDERVOICE_REFINE_REPEATS"].flatMap(Int.init) ?? 3
    )
    let transcriber = WhisperTranscriber(
        configuration: configuration,
    ...
    // A real-speech corpus need not be English. BUILDERVOICE_ACCURACY_CORPUS_LANGUAGE
    ...
    let corpusProfile = environment["BUILDERVOICE_ACCURACY_CORPUS_LANGUAGE"]
    ...
    report()
    report("BuilderVoice accuracy — model \(configuration.modelID)")
    if let modelPath = environment["BUILDERVOICE_MODEL_PATH"] {
        report(
            "model artifact: "
                + URL(fileURLWithPath: modelPath).standardizedFileURL.path
        )
    }
    if let corpusPath = environment["BUILDERVOICE_ACCURACY_CORPUS"] {
        report(
            "corpus input: "
    ...
    if flag("BUILDERVOICE_ACCURACY_MULTIENGINE") {
        _ = measureEngines(
            clips: clips,
            languageProfile: .english,
            gain: gain,
    ...
    // raw transcript is.
    if !flag("BUILDERVOICE_ACCURACY_SKIP_REFINE") {
        ...
        // Real recordings, from the same BUILDERVOICE_ACCURACY_CORPUS directory the
        ...
        // just removed.
        let corpusClips = environment["BUILDERVOICE_ACCURACY_CORPUS"]
            .map { URL(fileURLWithPath: $0) }
        ...
        // before it starts. Enable with BUILDERVOICE_REFINE_STRICT=1.
    ...
    //
    // Answers one question: could BuilderVoice send English dictation to a Whisper
    ...
    //
    // Opt-in: BUILDERVOICE_PROBE_ROUTING=1.
    if flag("BUILDERVOICE_PROBE_ROUTING"), Fixtures.isHindiVoiceInstalled() {
    ...
    if flag("BUILDERVOICE_ACCURACY_REQUIRE_REAL") {
...
// BUILDERVOICE_REFINE_PROBE=1 answers the scaling question on its own, in seconds
// rather than the minutes a full transcription pass costs.
validateScoring()
let outcome: Bool
if flag("BUILDERVOICE_SCORING_ONLY") {
    report("BuilderVoiceAccuracyChecks deterministic scoring passed.")
    outcome = true
} else if flag("BUILDERVOICE_CORPUS_VALIDATE_ONLY") {
    outcome = validateCorpusInput()
} else if flag("BUILDERVOICE_ACCURACY_SMOKE") {
    outcome = runRealSpeechSmoke()
} else if flag("BUILDERVOICE_STRUCTURE_PROBE") {
    outcome = probeSpokenStructure()
} else if let engineID = environment["BUILDERVOICE_ACCURACY_ENGINE"] {
    outcome = engineID == "list"
        ? listEngineBaselineCandidates()
        : measureEngineBaseline(engineID: engineID)
} else if let corpus = environment["BUILDERVOICE_REFINE_TEXTEVAL"] {
