...
# BSD mv nests into an existing directory, so clear the target first.
rm -rf zen-polish-4bit
if grep -q '"quantization"' fused/config.json; then
  mv fused zen-polish-4bit
else
  python3 -m mlx_lm convert --hf-path fused --mlx-path zen-polish-4bit --quantize
fi
